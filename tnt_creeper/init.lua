...
local tnt_node = minetest.registered_items["tnt:tnt_burning"]
...
local function spawn_snow(pos,self)
	if minetest.get_modpath("tnt") and tnt and tnt.boom and not minetest.is_protected(pos, "") then
		...
		if obj then
			obj:get_luaentity():set_node(tnt_node)
			obj:setvelocity({x=math.random(-5,5),y=math.random(5,10),z=math.random(-5,5)})
		end
		end
	else
		minetest.sound_play(self.sounds.explode, {
...
mobs:register_mob("tnt_creeper:tnt_creeper", {
	type = "monster",
	hp_min = 20,
	hp_max = 20,
...
mobs:spawn_specific("tnt_creeper:tnt_creeper",{ "default:dirt_with_snow", "default:snowblock", "default:snow" }, {"air"}, 0, 7, 20, 5500, 10, mobs_mc.spawn_height.overworld_min, mobs_mc.spawn_height.overworld_max)

-- compatibility
--mobs:alias_mob("mobs:creeper", "mobs_mc:tnt_creeper")

-- spawn eggs
mobs:register_egg("tnt_creeper:tnt_creeper", S("TNT Creeper"), "mobs_tnt_creeper_inv.png", 0)

if minetest.settings:get_bool("log_mods") then
	minetest.log("action", "TNT Creeper loaded")
end
