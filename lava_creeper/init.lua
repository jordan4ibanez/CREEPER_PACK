...
local function lava_dome(pos,self)
	local lava_radius = self.explosion_radius * 2

	for z=-lava_radius, lava_radius do
	for y=-lava_radius, lava_radius do
	for x=-lava_radius, lava_radius do
	  if x*x+y*y+z*z <= lava_radius*lava_radius + lava_radius then
		local index_pos = {x=pos.x+x,y=pos.y+y,z=pos.z+z}
		if minetest.get_node(index_pos).name == "air" then
			minetest.set_node(index_pos,{name="default:lava_source"})
		end
	  end
	end
	end
...
mobs:register_mob("lava_creeper:lava_creeper", {
	type = "monster",
	hp_min = 20,
	hp_max = 20,
	...
	do_custom = function(self, dtime)
		...
		if self.timer > self.explosion_timer - 0.5 then
			self.object:remove()
			lava_dome(self.object:getpos(),self)
			--[[
...
mobs:spawn_specific("lava_creeper:lava_creeper",{ "default:lava_source", "default:lava_flowing" }, {"air"}, 0, 7, 20, 5500, 10, -31000, 31000)

-- compatibility
--mobs:alias_mob("mobs:creeper", "mobs_mc:lava_creeper")

-- spawn eggs
mobs:register_egg("lava_creeper:lava_creeper", S("Lava Creeper"), "mobs_lava_creeper_inv.png", 0)

if minetest.settings:get_bool("log_mods") then
	minetest.log("action", "Lava Creeper loaded")
end
